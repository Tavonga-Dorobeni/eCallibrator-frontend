        ...
        >
          ...
          <template v-slot:footer>
            <Button
              text="Submit"
              btnClass="btn-dark "
              @click="handleNew(tool_type); $refs.modal2.closeModal()"
            />
          </template>
        </Modal>
      </div>
    </div>
    <GridSkletion :count="tool_types.length" v-if="isSkeletion" />
    <TableSkeltion :count="tool_types.length" v-if="isSkeletion2" />
...
import ToolTypeAddmodal from "./AddToolType";
import updateModal from "./EditToolType";
import List from "./ToolType-list";
import Grid from "./ToolTypes-grid";
import { useStore } from "vuex";
const store = useStore();

let fillter = ref("grid");
const openToolType = () => {
  store.dispatch("openToolType");
};
...
const handleNew = (new_sec) => {
  store.dispatch("createToolType", new_sec)
  .then(data =>{
