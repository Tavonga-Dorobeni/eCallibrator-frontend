<template>
  <div class="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
    <Card bodyClass="p-6" v-for="(item, i) in tool_types" :key="i">
      <!-- header -->
...
const actions = ref([
  ...
  },
  {
    ...
    doit: (data) => {
      ...
      store.dispatch('deleteToolType', data)
      .then(response => {
...
const updateTyp = () => {
  const toast = useToast();
  store.dispatch('updateToolType', currentTyp.value)
  .then(response => {
