<template>
  <div>
    <Card title="Section List" noborder>
      <Modal
        title="Edit Section"
        label=""
      ...
      >
        ...
        <Textinput
          label="Description"
          type="text"
          v-model="currentSec.Description"
      ...
      <vue-good-table
        ...
        :rows="this.$store.state.section.sections"
        :pagination-options="{
          enabled: true,
          perPage: perpage,
        }"
      ...
      >
        ...
        <template #pagination-bottom="props">
          <div class="py-4 px-3 justify-center flex">
            <Pagination
              :total="$store.state.section.sections.length"
              :current="current"
              :per-page="perpage"
              :pageRange="pageRange"
...
export default {
      ...
      actions: [
        ...
        },
        {
          ...
          doit: (data) => {
            ...
            this.$store.dispatch('deleteSection', data)
            .then(response => {
  ...
  methods: {
    updateSec(){
      const toast = useToast();
      this.$store.dispatch('updateSection', this.currentSec)
      .then(response => {
