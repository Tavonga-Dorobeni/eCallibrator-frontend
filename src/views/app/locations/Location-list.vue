<template>
  <div>
    <Card title="Location List" noborder>
      <Modal
        title="Edit Location"
        label=""
      ...
      >
        ...
        <Textinput
          label="Description"
          type="text"
          v-model="currentLoc.Description"
      ...
      <vue-good-table
        ...
        :rows="this.$store.state.location.locations"
        :pagination-options="{
          enabled: true,
          perPage: perpage,
        }"
      ...
      >
        ...
        <template #pagination-bottom="props">
          <div class="py-4 px-3 justify-center flex">
            <Pagination
              :total="$store.state.location.locations.length"
              :current="current"
              :per-page="perpage"
              :pageRange="pageRange"
...
export default {
  ...
  methods: {
    updateLoc(){
      const toast = useToast();
      this.$store.dispatch('updateLocation', this.currentLoc)
      .then(response => {
