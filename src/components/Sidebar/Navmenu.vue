<template>
  <ul>
    <!-- <li
      v-for="(item, i) in items.filter(i => i.roles.includes(currentUser.roles[0]) || i.roles.includes(currentUser.roles[1]))"
      :key="i"
      :class="`
...
export default {
  ...
  watch: {
    $route() {
      if (this.$store.state.mobielSidebar) {
        this.$store.state.mobielSidebar = false;
      }
